    ...
    <nav className="w-full border-b border-b-black/10 fixed top-0 left-0 bg-white z-50">
      <div className="think-arq-container">
        <div className="w-full flex items-center justify-between py-4">
          ...
          <div className="w-fit h-fit flex gap-8 text-lg font-medium">
              ...
              if (link?.dropDown?.length == 0) {
                return (
                  <Link
                    key={link.id}
                    href={link.href}
                    className="font-space-grotesk font-medium text-lg hover:text-orange-500 flex items-center justify-center">
                    {link.label}
                  </Link>
                );
              } else {
